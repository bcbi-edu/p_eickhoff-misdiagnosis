...
matcher = QuickUMLS(quickumls_fp='/Users/cindyli/Documents/CS/Research/workspace-biomed/pubmeddata/umls', overlapping_criteria="length", accepted_semtypes={"T047", "T191"})
generalFile = '/Users/cindyli/Documents/CS/Research/workspace-biomed/pubmeddata/basicversion/stats+info/diseases+count.txt'
zeroFile = '/Users/cindyli/Documents/CS/Research/workspace-biomed/pubmeddata/basicversion/stats+info/zero-zero.txt'
oneFile = '/Users/cindyli/Documents/CS/Research/workspace-biomed/pubmeddata/basicversion/stats+info/new-one-one-test1.txt'
titleFile = '/Users/cindyli/Documents/CS/Research/workspace-biomed/pubmeddata/basicversion/stats+info/titles.txt'
graphStatsFile = '/Users/cindyli/Documents/CS/Research/workspace-biomed/pubmeddata/basicversion/stats+info/graph-degrees-weighted1.txt'
...
# given a title, gets the matches and stores them as two lists, before and after, and returns these two lists as a tuple
# return (before, after), where before is the list of matches before the pattern and after is the list of matches after the pattern
...
# updates the counts of pairs
def handleOneOne(befDis, aftDis):
...
"""           
def handleParents(befDis, aftDis):
    simDis1Par, simDis1Chd, simDis1Syn = getParents.parCUIList(befDis)
    simDis2Par, simDis2Chd, simDis2Syn = getParents.parCUIList(aftDis)
    dis1 = befDis
    dis2 = aftDis
    ...
    if dis1 != befDis and dis2 != aftDis:
        print(befDis)
        getNormal(dis1)
        handleOneOne(dis1, dis2)
    elif dis1 != befDis:
        print(befDis)
        checkParents([dis1], simDis2Par, dis1, dis2, [dis1], simDis2Chd)
    elif dis2 != aftDis:
        print(aftDis)
        checkParents(simDis1Par, [dis2], dis1, dis2, simDis1Chd, [dis2])
    else:
        print("c4")
        checkParents(simDis1Par, simDis2Par, befDis, aftDis, simDis1Chd, simDis2Chd)
        
    
"""
    checks if parent CUIs are in dictionary and adds counts to parents
    if children CUIs are in dictionary, adds children counts to it and removes children
"""
def checkParents(parList1, parList2, befDis, aftDis, chdList1, chdList2):
    dis1 = befDis
    dis2 = aftDis
...
"""
    checks the children in chdList1 of befDis to see if it already exists and adds to parent if yes
"""                    
...
"""
    writes to the zero-zero file if QuickUMLS extracts zero diseases on one side of the pattern
...
"""    
def handleOther(bef, aft, title, disDict):
    disFile.write(title + "\n")
    for dis1 in bef:
        for dis2 in aft:
...
def runMatches():
    for line in file:
        ...
        if len(matches[0]) == 1 and len(matches[1]) == 1:
            print("check0")
            #handleOneOne(matches[0][0].get("cui"), matches[1][0].get("cui"))
            handleParents(matches[0][0].get("cui"), matches[1][0].get("cui"))
            print("check0.5")
        #elif len(matches[0]) == 0 or len(matches[1]) == 0:
         #   handleZeros(matches[0], matches[1], line)
        #handleOther(matches[0], matches[1], line.lower(), nonOneDis)

"""
    to get a graph of related nodes 
    (aka all the nodes connected to one disease and all the nodes connected to its connections, and so on)
"""     
...
"""topIn = sorted(inDeg.items(), key = lambda x : x[1])
topOut = sorted(outDeg.items(), key = lambda x : x[1])

top20In = topIn[-20:]
top20Out = topOut[-20:]

curVal = 0
graphStats.write("Top In Degrees\n")
for x in top20In:
    curVal = x[1]
    graphStats.write(x[0] + ": " + str(x[1]) + "\n     out-degree: " + str(outDeg.get(x[0])) + "\n\n")
for x in topIn[:-20]:
    if x[1] == curVal:
        graphStats.write(x[0] + ": " + str(x[1]) + "\n     out-degree: " + str(outDeg.get(x[0])) + "\n\n")
...
curVal = 0
graphStats.write("Top Out Degrees\n")
for x in top20Out:
    curVal = x[1] 
    graphStats.write(x[0] + ": " + str(x[1]) + "\n     in-degree: " + str(inDeg.get(x[0])) + "\n\n")
for x in topOut[:-20]:
    if x[1] == curVal:
        graphStats.write(x[0] + ": " + str(x[1]) + "\n     in-degree: " + str(inDeg.get(x[0])) + "\n\n")"""
...
nx.draw(G,pos, edge_color=weights, width=1, node_color = 'b', with_labels=False, node_size=[v * 10000 + 10 for v in d.values()], font_size=8, edge_cmap=plt.cm.get_cmap('Purples'))
